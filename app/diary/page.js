...
import { ChevronLeft, ChevronRight, Plus, Search, X, Camera, Trash2, Settings2 } from 'lucide-react'
...
export default function Diary() {
  ...
  const [formItem, setFormItem] = useState({
    id: null, name: '', image_url: '', amount: 100,
  ...
  const clearHistory = () => {
    if (!confirm('确定清空所有最近添加记录吗?')) return
    setHistory([])
    localStorage.removeItem('diary_food_history')
  }
  ...
  const handleTouchStart = (name) => {
    const timer = setTimeout(() => {
        if(confirm(`删除 "${name}" ?`)) {
            setHistory(prev => {
                const nw = prev.filter(h => h.name !== name)
                localStorage.setItem('diary_food_history', JSON.stringify(nw))
                return nw
            })
        }
        setPressTimer(null)
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const filePath = `diary/${Math.random()}.${fileExt}`
  ...
  const handleSaveGoal = async () => {
    ...
    const { error } = await supabase.from('daily_goals').upsert({
      user_id: user.id, apply_date: date,
      target_protein: tempGoals.p, target_fat: tempGoals.f, target_carbs: tempGoals.c
  ...
  const handleSave = async () => {
    ...
    let payload = {
      ...
      amount_g: parseFloat(formItem.amount)
    }
    ...
    const ratio = parseFloat(formItem.amount) / 100

    if (mode === 'edit') {
       payload.intake_calories = formItem.density.cal * parseFloat(formItem.amount)
       payload.intake_protein = formItem.density.p * parseFloat(formItem.amount)
       payload.intake_fat = formItem.density.f * parseFloat(formItem.amount)
       payload.intake_carbs = formItem.density.c * parseFloat(formItem.amount)
       await supabase.from('daily_logs').update(payload).eq('id', formItem.id)
    } else {
       payload.intake_calories = formItem.calories * ratio
       payload.intake_protein = formItem.protein * ratio
       payload.intake_fat = formItem.fat * ratio
       payload.intake_carbs = formItem.carbs * ratio
  ...
  const handleDelete = async () => {
    ...
    await supabase.from('daily_logs').delete().eq('id', formItem.id)
    closeModal()
    fetchLogs()
  }
  ...
  const openEditModal = (log) => {
    ...
    setFormItem({
      id: log.id, name: log.food_name, image_url: log.image_url, amount: amount,
      calories: log.intake_calories * factor,
      protein: log.intake_protein * factor,
      fat: log.intake_fat * factor,
      carbs: log.intake_carbs * factor,
      density: { cal: log.intake_calories/amount, p: log.intake_protein/amount, f: log.intake_fat/amount, c: log.intake_carbs/amount },
      type: 'unknown'
    })
    setShowModal(true)
  }
  ...
  const closeModal = () => {
    setShowModal(false); setMode('search'); setSearch('')
    setFormItem({ id:null, name:'', image_url:'', amount:100, calories:0, protein:0, fat:0, carbs:0, density:{}, type:'ingredient' })
  }

  const preview = (() => {
    const ratio = (parseFloat(formItem.amount) || 0) / 100
    return {
      cal: formItem.calories * ratio,
      p: formItem.protein * ratio,
      f: formItem.fat * ratio,
      c: formItem.carbs * ratio
    }
  })()
  ...
  const ProgressBar = ({ current, target, colorClass }) => {
    ...
    return (
      <div className="h-2 w-full bg-white/60 rounded-full overflow-hidden mt-2 ring-1 ring-black/5">
    ...
    <>
        ...
        <div className="space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-6 relative z-10">
              ...
              <button 
                onClick={() => { setTempGoals(goals); setShowGoalModal(true) }}
                className="group flex items-center gap-1.5 text-xs font-bold bg-slate-50 text-slate-600 px-3 py-2 rounded-xl hover:bg-slate-100 transition-colors"
              >
              ...
              {['carbs', 'protein', 'fat'].map(k => (
                <div key={k} className={`${COLORS[k].bg} p-4 rounded-2xl border ${COLORS[k].border.replace('focus:', '')} border-opacity-20 flex flex-col items-center justify-center text-center`}>
                  ...
                  <div className="w-full">
                    <ProgressBar current={summary[k[0]]} target={goals[k[0]]} colorClass={COLORS[k].bar} />
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={type} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
                  <div className="flex justify-between items-center mb-5 pb-4 border-b border-slate-50">
                     <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-slate-50 rounded-full flex items-center justify-center text-xl shadow-inner">
                          {conf.emoji}
                        </div>
                        <div>
                          <h3 className="font-bold text-lg text-slate-900 leading-none">{conf.label}</h3>
                          <p className="text-[10px] text-slate-400 mt-1 font-medium">{conf.sub}</p>
                        </div>
                     </div>
                  ...
                  <div className="space-y-3">
                    {typeLogs.map(log => (
                      <div key={log.id} onClick={() => openEditModal(log)} className="group flex justify-between items-center p-2 rounded-2xl hover:bg-slate-50 transition-colors cursor-pointer -mx-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-400 mt-0.5">
                               ...
                               <div className="flex gap-1.5">
                                 <span className="text-indigo-400">碳水:{fmt(log.intake_carbs)}</span>
                                 <span className="text-amber-400">蛋白质:{fmt(log.intake_protein)}</span>
                                 <span className="text-rose-400">脂肪:{fmt(log.intake_fat)}</span>
                               </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 pl-2">
                            ...
                            <button 
                                onClick={(e) => handleQuickDelete(e, log.id)} 
                    ...
                    {typeLogs.length === 0 && (
                      <div className="py-6 text-center">
                        <div className="text-2xl opacity-20 grayscale mb-2">{conf.emoji}</div>
                        <div className="text-xs text-slate-300 font-medium">还没有记录{conf.label}</div>
                      </div>
                    )}
                  </div>

                  <button 
                    onClick={() => { setMealType(type); setMode('search'); setShowModal(true) }}
                    className="mt-4 w-full py-3 flex items-center justify-center gap-2 text-xs font-bold text-slate-400 hover:text-white hover:bg-black rounded-xl transition-all border border-dashed border-slate-200 hover:border-transparent group"
                  >
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in zoom-in-95 duration-200">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl">
            ...
            <div className="space-y-5">
              {[
                { k:'c', key: 'c', ...COLORS.carbs },
                { k:'p', key: 'p', ...COLORS.protein },
                { k:'f', key: 'f', ...COLORS.fat }
              ].map(item => (
                <div key={item.key}>
                  <label className={`text-xs font-bold ${item.text} uppercase tracking-wider mb-1 block ml-1`}>{item.label} (g)</label>
                  <input 
                    type="number" 
                    onFocus={(e)=>e.target.select()} 
                    className={`w-full bg-slate-50 border-b-2 border-slate-100 p-3 rounded-t-xl text-xl font-bold outline-none ${item.border} transition-colors text-slate-900`} 
                    value={tempGoals[item.key]} 
                    onChange={e=>setTempGoals({...tempGoals, [item.key]:e.target.value})}
                  />
                </div>
              ))}
              
              <div className="bg-slate-900 text-white p-4 rounded-2xl text-center mt-4 shadow-lg shadow-slate-900/20">
                <div className="text-xs opacity-60 mb-1 uppercase tracking-widest">预计总热量</div>
                <div className="text-2xl font-black">{(tempGoals.c*4 + tempGoals.p*4 + tempGoals.f*9)} <span className="text-sm font-normal opacity-60">kcal</span></div>
              </div>

              <div className="flex gap-3 mt-4">
                <button onClick={() => setShowGoalModal(false)} className="flex-1 py-3.5 text-slate-500 font-bold hover:bg-slate-50 rounded-xl transition-colors">取消</button>
                <button onClick={handleSaveGoal} className="flex-1 bg-black text-white py-3.5 rounded-xl font-bold shadow-lg hover:bg-slate-800 transition-all">保存设置</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showModal && (
        <div className={`fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] flex items-center justify-center sm:p-4 animate-in slide-in-from-bottom-10 duration-300`}>
          {/* 核心修改: 动态容器尺寸 
              如果是 detail/edit 模式，使用 sm:max-w-sm (小号) + sm:h-auto (自动高度)
              否则(search模式)，保持 sm:max-w-2xl + sm:h-[85vh] (大号)
          */}
          <div className={`
            bg-white w-full overflow-hidden shadow-2xl flex flex-col relative
            ${isCompactMode 
                ? 'h-auto sm:max-w-sm sm:rounded-3xl' // 紧凑模式
                : 'h-full sm:h-[85vh] sm:max-w-2xl sm:rounded-3xl' // 搜索模式
            }
          `}>
            
            <button onClick={closeModal} className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full z-20 transition-colors text-slate-500">
              <X size={20}/>
            </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 scroll-smooth">
              ...
              {mode === 'search' && !formItem.name && (
                <div className="mt-8 text-center max-w-lg mx-auto">
                   <div className="w-16 h-16 bg-slate-50 rounded-2xl mx-auto flex items-center justify-center text-slate-300 mb-6">
                     <Search size={32}/>
                   </div>
                   ...
                   <div className="relative mb-8">
                     <input 
                       autoFocus 
                       className="w-full bg-slate-50 p-4 rounded-2xl text-lg font-bold text-center outline-none focus:ring-2 focus:ring-black/5 focus:bg-white transition-all text-slate-900 placeholder:text-slate-300"
                       placeholder="搜索菜谱、食材、零食..."
                       value={search}
                   ...
                   {!search && history.length > 0 && (
                     <div className="text-left animate-in fade-in slide-in-from-bottom-2 mb-8">
                       <div className="flex justify-between items-end mb-3 px-1">
                         <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">最近添加</h4>
                         <button onClick={clearHistory} className="text-xs text-slate-300 hover:text-rose-500 transition-colors">
                           清空
                         </button>
                       </div>
                       <div className="flex flex-wrap gap-2">
                         {history.map(item => (
                           <button
                             key={item.name}
                             onClick={() => !pressTimer && handleSelectSearchResult(item, item.type)}
                             onTouchStart={() => handleTouchStart(item.name)}
                             onTouchEnd={handleTouchEnd}
                             className="bg-white border border-slate-200 px-4 py-2 rounded-xl text-sm font-bold text-slate-700 shadow-sm hover:border-black hover:shadow-md active:scale-95 transition-all select-none"
                           >
                             {item.name}
                           </button>
                         ))}
                   ...
                   {search && (
                     <div className="space-y-6 text-left animate-in fade-in slide-in-from-bottom-2">
                        {searchResults.recipes.length > 0 && (
                          <div>
                            <h4 className="text-xs font-bold text-slate-400 uppercase mb-3 pl-1">我的菜谱</h4>
                            <div className="space-y-2">
                              {searchResults.recipes.map(r=>(
                                <button key={r.id} onClick={()=>handleSelectSearchResult(r,'recipe')} className="w-full p-3 bg-white border border-slate-100 rounded-xl flex items-center gap-4 hover:border-black transition-all group">
                                  <div className="w-12 h-12 bg-slate-100 rounded-lg bg-cover bg-center shrink-0" style={{backgroundImage:`url(${r.cover_image})`}}></div>
                                  <div className="text-left">
                                    <div className="font-bold text-slate-900 group-hover:text-black">{r.title}</div>
                                    <div className="text-xs text-orange-500 font-medium">整道菜</div>
                                  </div>
                                </button>
                              ))}
                            </div>
                        ...
                        {(searchResults.ingredients.length > 0 || searchResults.products.length > 0) && (
                          <div>
                            <h4 className="text-xs font-bold text-slate-400 uppercase mb-3 pl-1">食材库</h4>
                            <div className="space-y-2">
                              {[...searchResults.ingredients, ...searchResults.products].map(i => (
                                <button key={i.id} onClick={()=>handleSelectSearchResult(i, i.calories ? 'product' : 'ingredient')} className="w-full p-4 bg-white border border-slate-100 rounded-xl flex justify-between items-center hover:border-black transition-all group">
                                   <div className="text-left">
                                      <div className="font-bold text-slate-900 group-hover:text-black">{i.name}</div>
                                      <div className="text-xs text-slate-400 mt-0.5">{fmt(i.calories)} kcal/100g</div>
                                   </div>
                                   <Plus size={16} className="text-slate-300 group-hover:text-black"/>
                                </button>
                              ))}
                            </div>
                          </div>
                        ...
                        {searchResults.recipes.length===0 && searchResults.ingredients.length===0 && searchResults.products.length===0 && (
                          <div className="text-center py-8">
                            <p className="text-slate-400 mb-4">未找到 "{search}"</p>
                            <button onClick={()=>{setMode('custom');setFormItem(prev=>({...prev,name:search}))}} className="bg-black text-white px-6 py-3 rounded-xl font-bold hover:bg-slate-800 transition-colors">
                              创建自定义食物
                            </button>
                          </div>
                        )}
              ...
              {/* Detail / Edit Mode (全面紧凑化) */}
              {(mode === 'detail' || mode === 'edit') && (
                 <div className="mx-auto pt-2 text-center">
                    {/* 图片缩小: w-20 h-20 */}
                    <div className="w-20 h-20 bg-slate-100 mx-auto rounded-xl mb-3 flex items-center justify-center text-3xl shadow-inner overflow-hidden border border-slate-100">
                       {formItem.image_url ? <img src={formItem.image_url} className="w-full h-full object-cover"/> : <span className="opacity-50 grayscale">🥘</span>}
                    </div>
                    ...
                    <div className="flex justify-center gap-2 mb-4">
                       <span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-lg text-xs font-bold">C: {fmt(formItem.carbs)}</span>
                       <span className="bg-amber-50 text-amber-700 px-2 py-0.5 rounded-lg text-xs font-bold">P: {fmt(formItem.protein)}</span>
                       <span className="bg-rose-50 text-rose-700 px-2 py-0.5 rounded-lg text-xs font-bold">F: {fmt(formItem.fat)}</span>
                    </div>
                    
                    {/* 输入框区域紧凑化: p-3, mb-4 */}
                    <div className="bg-slate-50 p-3 rounded-2xl mb-4 relative border border-slate-100">
                       <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">摄入重量 (g)</label>
                       {/* 字体减小: text-3xl */}
                       <input 
                         type="number" 
                         autoFocus
                         onFocus={(e)=>e.target.select()}
                         className="w-full bg-transparent text-center text-3xl font-black text-slate-900 outline-none p-0 placeholder:text-slate-200"
                         value={formItem.amount}
                         onChange={e => setFormItem({...formItem, amount: e.target.value})}
                       />
                       <div className="text-[10px] text-slate-400 mt-1 font-medium">
                         每100g含 {fmt(formItem.calories)} kcal
                       </div>
                    </div>

                    {/* 结果展示紧凑化 */}
                    <div className="bg-black text-white p-3 rounded-2xl shadow-xl shadow-black/20 mb-4 flex flex-col items-center justify-center">
                       <div className="text-[10px] opacity-60 mb-0.5 font-medium tracking-wide">本次热量</div>
                       <div className="text-2xl font-black flex items-baseline gap-1">
                         {fmt(preview.cal)} <span className="text-xs font-medium opacity-50">kcal</span>
                       </div>
                    </div>

                    <div className="flex gap-3">
                       {mode === 'edit' && (
                         <button onClick={handleDelete} className="p-3 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-100 transition-colors">
                           <Trash2 size={18}/>
                         </button>
                       )}
                       <button onClick={handleSave} className="flex-1 bg-slate-900 text-white py-2.5 rounded-xl font-bold text-sm hover:scale-[1.02] active:scale-95 transition-all shadow-lg">
                         {mode === 'edit' ? '更新记录' : '确认添加'}
                       </button>
                    </div>
                 </div>
                 ...
                 <div className="max-w-md mx-auto pt-4 space-y-6">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <input 
                          className="w-full text-2xl font-bold border-b-2 border-slate-200 py-2 outline-none bg-transparent text-slate-900 placeholder:text-slate-300 focus:border-black transition-colors" 
                          placeholder="例如: 康师傅" 
                          value={formItem.name} 
                   ...
                   <div className="grid grid-cols-3 gap-4">
                      {['carbs','protein','fat'].map(k=>(
                        <div key={k} className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                          <label className="text-[10px] text-slate-400 uppercase font-bold block mb-2">{k === 'carbs' ? '碳水' : k === 'protein' ? '蛋白质' : '脂肪'} (g)</label>
                   ...
                   <div className="bg-slate-900 text-white p-5 rounded-2xl text-center">
                      <div className="text-xs opacity-60 uppercase mb-1">估算热量 (100g)</div>
                   ...
                   <div className="border-t border-slate-100 pt-6">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-2 text-center">本次摄入量 (g)</label>
                      <input type="number" onFocus={(e)=>e.target.select()} className="w-full text-center font-black text-4xl outline-none bg-transparent text-slate-900" value={formItem.amount} onChange={e=>setFormItem({...formItem, amount:e.target.value})}/>
                   </div>
